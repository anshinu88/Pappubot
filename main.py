...
# ------------- CONFIG (YAHAN APNI NAYI DETAILS DAALO) -------------
DISCORD_TOKEN = "MTQ0MjU0Nzc2Nzk0NjkwMzU3Mg.G_-__h.WMee9JQPeUVayuA9kj3drjQpzxUim_JaUkbL9c"  # yahan naya Discord bot token
GEMINI_API_KEY = "AIzaSyDa-Vke3GnRZtGxKg8UJExhhTBuFqGSQas"    # yahan nayi Gemini API key (AI Studio se)
OWNER_ID = 1133366983870652476                     # tera Discord user ID (numbers only)
# ------------------------------------------------------------------
...
async def handle_owner_nl_admin(message: discord.Message, clean_text: str) -> bool:
    ...
    if (
        "unmute" in text
        or ("mute" in text and any(w in text for w in ["hata", "hatta", "remove", "nikal"]))
    ):
    ...
    # ---------- MUTE (Muted role add) ----------
    if any(k in text for k in ["mute", "moot", "chup"]) and "unmute" not in text:
        if not target_member:
